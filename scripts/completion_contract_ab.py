...
async def drain_background_tasks(agent) -> None:
    """`chat()` schedules memory extraction onto `_bg_tasks`. Left running it
    overlaps the next trial -- breaking "a live harness runs alone" and
    polluting the latency numbers. Snapshot first: the done-callback mutates
    the set while it drains."""
...
def report(rows: list[dict], label: str) -> dict:
    ...
    for arm, arm_rows in per_arm.items():
        ...
        first = [r["time_to_first_visible_s"] for r in arm_rows
                 if r.get("time_to_first_visible_s")]
        ...
        print(f"   timed out / errored    {_rate(arm_rows, 'timed_out')[0]} / "
              f"{sum(1 for r in arm_rows if r.get('error'))}")
        print(f"   tool rounds (mean)     {sum(r['tool_round_count'] for r in arm_rows)/max(n,1):.2f}")
        print(f"   latency p50/p90        {_pct(lat, .5)}s / {_pct(lat, .9)}s")
        if first:
            print(f"   first-visible p50/p90  {_pct(first, .5)}s / {_pct(first, .9)}s")
...
async def main() -> int:
    ...
    parser.add_argument("--label", default="pilot", choices=["pilot", "confirmation", "smoke"])
